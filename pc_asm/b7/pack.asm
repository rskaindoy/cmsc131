...
_asm_main:
        ...
        ; mask version to 4 bits
        mov     eax, esi
        and     eax, 0x0F
        shl     eax, 12                ; version -> bits 12-15
        mov     edx, eax

        ; Mask flag to 1 bit
        mov     eax, edi
        and     eax, 0x01
        shl     eax, 8                 ; flag -> bit 8
        or      edx, eax

        ; mask length to 8 bits
        mov     eax, ecx
        and     eax, 0xFF
        or      edx, eax

        ; store packed value in memory
        mov     [packed], edx

        mov     eax, packed_msg
        call    print_string

        ; print the packed value. Read it back from memory with [packed].
	mov 	eax, [packed]
	call	print_int
        call    print_nl
